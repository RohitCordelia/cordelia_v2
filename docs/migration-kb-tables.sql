...
-- ──────────────────────────────────────────────────────────────────────────────
-- Migration v2: idempotent, version-aware ingestion
-- Adds a stable upsert key + content/version markers so re-seeding is safe and a
-- model/chunker change can trigger a controlled re-index. Idempotent — safe to
...
-- Backfill source_key for any legacy rows, then enforce NOT NULL + uniqueness.
UPDATE kb_document SET source_key = id::text WHERE source_key IS NULL;
ALTER TABLE kb_document ALTER COLUMN source_key SET NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS kb_document_source_key_uniq ON kb_document(source_key);

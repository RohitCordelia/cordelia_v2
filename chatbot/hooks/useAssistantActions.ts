...
import { AssistantResponse, AssistantAction, ItineraryDetail, CabinSelection, ExtractedFilters, BotPageContext } from '../types';
...
const NOOP = () => {};
const NOOP_FILTER: (f: ExtractedFilters) => void = () => {};
...
export function useAssistantActions(opts: UseAssistantActionsOptions) {
    ...
    const onApplyFilters = search?.onApplyFilters ?? opts.onApplyFilters ?? NOOP_FILTER;
    const onResetFilters = search?.onResetFilters ?? opts.onResetFilters ?? NOOP;
    ...
    const onSelectCabin = cabin?.onSelectCabin ?? opts.onSelectCabin;
    const isLoading = search?.results.isLoading ?? opts.isLoading ?? false;
    const itineraryCount = search?.results.totalCount ?? opts.itineraryCount ?? 0;
    const { addBotMessage, speakText, userId } = opts;
    ...
    const hasCabinDataRef = useRef(false);
    hasCabinDataRef.current = (cabin?.cabins?.length ?? 0) > 0;
    const itineraryIdsRef = useRef<string[]>([]);
    itineraryIdsRef.current = (search?.results.itineraries ?? []).map(it => it.id).filter(Boolean);
    ...
    const buildContext = useCallback((): ExecutionContext => {
        ...
        return {
            ...
            hasItineraryDetail: !!currentDetail?.id,
            hasCabinData: hasCabinDataRef.current,
            availableItineraryIds: itineraryIdsRef.current,
        };
    }, []);

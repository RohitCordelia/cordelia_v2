...
function ChatScreen({
    ...
    const showSuggestions = suggestions && suggestions.length > 0 && !isTyping && !inputValue.trim();
                ...
                {showSuggestions && (
                    <div className="flex flex-wrap gap-1.5 px-3 py-2 border-t border-gray-100">
                        {suggestions!.map(s => (
                            <button
                                key={s.label}
                                className="text-xs px-3 py-1.5 rounded-full border border-gray-200 bg-white text-gray-700 hover:bg-gray-50 hover:border-gray-300 transition-colors whitespace-nowrap"

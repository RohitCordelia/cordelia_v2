...
export function itineraryDetailSection(detail: ItineraryDetail): string {
    ...
    return `[LIVE] THIS ITINERARY (user is currently on the view itinerary page for this specific sailing):
...
export function cabinSelectionSection(cabins: AvailableCabin[]): string {
    ...
    return `[LIVE] CABIN SELECTION (real-time availability and pricing from API).
${currentStr}
...
function knowledgeSection(knowledge: string): string {
    return `[KB] CORDELIA CRUISES KNOWLEDGE BASE (supplementary — [LIVE] sections above take precedence for pricing, availability, and offers):\n${knowledge}\nUse the above for general cruise questions. If the answer isn't here or in any [LIVE] section, say so briefly and suggest they check the website.`;
}
...
export function buildBookingPrompt(ctx: PromptContext): string {
    ...
    // query-matched KB — the compact global core still covers common FAQ.
    const relevantKnowledge = ctx.skipDetailedKb
        ? ''
        : getRelevantKnowledgeCapped(
              ctx.recentContext ? `${ctx.userMessage} ${ctx.recentContext}` : ctx.userMessage,
              400,
          );
    const sections = [
        identitySection(),

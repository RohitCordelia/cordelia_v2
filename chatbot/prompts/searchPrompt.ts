...
function itineraryContextSection(ctx: PromptContext): string {
    ...
    return `ITINERARIES ON PAGE (${valid.length}):
${lines.join('\n')}
...
function filterRulesSection(ctx: PromptContext): string {
    ...
    return `CURRENTLY AVAILABLE OPTIONS (live data — use these, NOT the knowledge base, when the user asks what is available):
...
${knowledge}
Use the above knowledge to answer accurately. If the answer isn't here, say so and suggest they call/check the website.`;
}

/**
 * KB Seed Script — seeds Cordelia knowledge base into pgvector / Pinecone
...
const KB_SECTIONS: Array<{ key: string; title: string; source: string; ship?: string }> = [
  { key: 'about',               title: 'About Cordelia Cruises',           source: 'cms' },
  { key: 'empress',             title: 'The Empress — Ship Overview',      source: 'cms',    ship: 'empress' },
  { key: 'sky',                 title: 'Cordelia Sky — Ship Overview',     source: 'cms',    ship: 'sky' },
  { key: 'empress_cabins',      title: 'Empress — Cabin Details',          source: 'cms',    ship: 'empress' },
  { key: 'sky_cabins',          title: 'Cordelia Sky — Cabin Details',     source: 'cms',    ship: 'sky' },
  { key: 'sky_restaurants',     title: 'Cordelia Sky — Restaurants & Bars',source: 'cms',    ship: 'sky' },
  { key: 'empress_restaurants', title: 'Empress — Restaurants & Venues',   source: 'cms',    ship: 'empress' },
  { key: 'entertainment',       title: 'Entertainment & Shows',            source: 'cms',    ship: 'empress' },
  { key: 'sky_decks',           title: 'Cordelia Sky — Deck Layout',       source: 'cms',    ship: 'sky' },
  { key: 'alcohol',             title: 'Alcohol Packages',                 source: 'policy', ship: 'empress' },
  { key: 'wifi',                title: 'Wi-Fi Packages',                   source: 'policy' },
  { key: 'inclusions',          title: "What's Included / Excluded",       source: 'faq' },
  { key: 'empress_policies',    title: 'Empress — Booking Policies',       source: 'policy', ship: 'empress' },
  { key: 'sky_policies',        title: 'Cordelia Sky — Booking Policies',  source: 'policy', ship: 'sky' },
  { key: 'destinations',        title: 'Destinations & Seasonal Weather',  source: 'cms' },
  { key: 'port_details',        title: 'Port Details',                     source: 'cms' },
  { key: 'parking',             title: 'Valet & Parking (Mumbai)',         source: 'faq' },
  { key: 'tender_port',         title: 'Tender Port Policy',               source: 'policy' },
  { key: 'general_policies',    title: 'General Policies',                 source: 'policy' },
  { key: 'offers',              title: 'Current Offers & Promotions',      source: 'cms' },
  { key: 'faq',                 title: 'Frequently Asked Questions',       source: 'faq' },
];
...
async function main() {
  ...
  console.log(`📚 Seeding ${KB_SECTIONS.length} KB sections...\n`);
  let totalChunks = 0;

  for (const section of KB_SECTIONS) {
    const content = KNOWLEDGE_BASE[section.key]?.trim();
    if (!content) {
      console.warn(`⚠️  Skipping ${section.key} — no content`);
      continue;
    }

    const metadata: Record<string, any> = {};
    if (section.ship) metadata.ship = section.ship;

    // Create document record
    const documentId = await kbService.upsertDocument({
    ...
    console.log(`📄 ${section.title} — ${chunks.length} chunk(s)`);

    // Embed in batches of 10 (Gemini rate limit safety)
    const batchSize = 10;
    for (let i = 0; i < chunks.length; i += batchSize) {
      const batch = chunks.slice(i, i + batchSize);
      const embeddings = await embeddingsService.embedBatch(batch);

      for (let j = 0; j < batch.length; j++) {
        await kbService.upsertChunk({
          documentId,
          chunkIndex: i + j,
          content: batch[j],
  ...
  console.log(`\n✅ Seeding complete — ${totalChunks} chunks stored across ${KB_SECTIONS.length} documents.`);
  await app.close();
  process.exit(0);
}

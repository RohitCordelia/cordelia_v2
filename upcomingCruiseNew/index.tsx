...
import { buildSearchPageContext } from '../../components/chatbot/buildPageContext';
...
export default function UpcomingCruise({}: Props) {
    ...
    const botPageContext = useMemo(() => buildSearchPageContext({
        portNames: portsList?.map((p: any) => p.name) || [],
        originNames: origin?.map((p: any) => p.name) || [],
        dates: availableDates || [],
        nights: nightList?.map((n: any) => Number(n)) || [],
        itineraryData,
        isLoading,
        isLoggedIn: false,
        onApplyFilters: applyFromChatbot,
        onResetFilters: resetFromChatbot,
    }), [portsList, origin, availableDates, nightList, itineraryData, isLoading, applyFromChatbot, resetFromChatbot]);

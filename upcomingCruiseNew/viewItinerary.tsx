...
import { ItineraryDetail } from '../../components/chatbot/types';
...
export default function ViewItinerary({ }: any) {
    ...
    const chatbotItineraryDetail = useMemo((): ItineraryDetail | undefined => {
        ...
        return {
            ...
            ship_name: itineraryData.ship?.name || '',
            route_name: itineraryData.route?.name || '',
            starting_fare: itineraryData.starting_fare,
            actual_starting_fare: itineraryData.actual_starting_fare,
            discount_pct: itineraryData.discount_pct || 0,
            offers_available: (itineraryData.offers_available || []).map((o: any) => typeof o === 'string' ? o : o?.offer).filter(Boolean),
            tags: itineraryData.tags || [],
            starting_port: itineraryData.starting_port?.name || '',
            destination_port: itineraryData.destination_port?.name || '',
            ports: (itineraryData.ports || []).filter((p: any) => p.name && p.name !== 'At Sea').map((p: any) => ({
                day: p.day || '',
                name: p.name,
                title: p.title || '',
                ...
                country: p.country || '',
                shore_excursions: (p.shore_excursions || []).filter((s: any) => s.active).map((s: any) => ({
                    title: s.title,
                    description: s.description || '',
                    active: s.active,
                })),
            })),
            is_tender_port: !!itineraryData.is_tender_port,
            is_international: !!itineraryData.is_international,
            has_shore_excursions: !!itineraryData.shore_excursions,
        };
    }, [itineraryData]);
  ...
  `}>
            ...
            <CruiseChatbot
                ...
                availableDates={[]}
                availableNights={[]}
                itineraryCount={itineraryData ? 1 : 0}
                isLoading={isLoading}
                itineraries={[]}
                onApplyFilters={() => {}}
                onResetFilters={() => {}}
                itineraryDetail={chatbotItineraryDetail}
                isLoggedIn={token ? true : false}
            />
            <Footer />
        </div>
